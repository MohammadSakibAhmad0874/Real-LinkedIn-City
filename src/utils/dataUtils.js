...
export function normaliseContributions(rawDays) {
  ...
  const cells = sorted.map(d => {
    ...
    return { ...d, week, day, type: d.type || null };
  });
...
/**
 * Filter cells by activity type.
 * Non-matching cells get count=0 and _dimmed=true so they render as flat tiles.
 * Matching cells keep full count. Grid shape is preserved.
 */
export function filterByType(cells, activityType) {
  if (!activityType || activityType === 'all') return cells;
  return cells.map(c => {
    if (c.count === 0) return c;
    if (c.type === activityType) return { ...c, _dimmed: false };
    return { ...c, count: 0, _dimmed: true, _originalCount: c.count };
  });
}
